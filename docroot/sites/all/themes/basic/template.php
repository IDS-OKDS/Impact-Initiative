...
function basic_preprocess_page(&$vars, $hook) {
	 ...
	 */
	$vars['hero_theme_term'] = '';
	if(isset($_GET['field_themes_tid'])){
		if(count($_GET['field_themes_tid']) == 1){
			$tid = $_GET['field_themes_tid'][0];
			$hero_theme_term = taxonomy_term_load($tid);
			if($hero_theme_term){
				$vars['hero_theme_term'] = $hero_theme_term;
			}
		}
	}
...
/**
 * Instance of hook_taxonomy_menu_block
 * Generate markup for our list.
 */
function basic_taxonomy_menu_block($variables) {
	...
	foreach ($tree as $tid => $term) {
		...
		$theme_color = $term_obj->field_theme_colour[LANGUAGE_NONE][0]['safe_value'];
		if(isset($_GET['field_themes_tid'])){
			if(in_array($tid, (count($_GET['field_themes_tid']) > 0 ? $_GET['field_themes_tid'] : array()))){
				$term['active_trail'] = 1;
			}
		}
		$i++;
		...
		$output .= '<li' . drupal_attributes($attributes) . '>' . basic_epd_convert_navigation_taxonomy_links(l($term['name'], $term['path'], $options = array('alias' => TRUE, 'attributes' => array('class' => array('w-nav-link', 'navlink'), 'style' => 'border-bottom-color: #' . $theme_color . ';'))));
...
function basic_epd_convert_navigation_taxonomy_links($path){
	return str_replace("taxonomy/term/","search?field_themes_tid[]=",$path);
}

function basic_epd_default_field_image_path($field_name) {
...
function basic_epd_get_field_image_path($field_name, $entity, $delta = 0) {
	$url = isset($entity->{$field_name}[LANGUAGE_NONE][$delta]['uri']) ? $entity->{$field_name}[LANGUAGE_NONE][$delta]['uri']:'';
	if($url){
...
function basic_epd_get_referenced_entity($entity_reference){
    ...
    foreach($entity_reference as $nid => $node_arr){
    	if(is_numeric($nid)){
    		$referenced_entity = node_load($nid);
    	}
    }
